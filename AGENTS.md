...
F2a vs F1: Spearman rho=0.435 p<0.0001 — moderate and expected co-variation from shared power-chain stress; no methodological change needed. Correlation becomes problematic only at very high overlap (roughly rho >= 0.8).
F2a by circuit: C1 p50=445 mV (few large spikes), C2 p50=145 mV (more frequent smaller events).

**Phase 2 physics caveat for Phase 3 scoring**: equal weighting between LED-side and driver-side sub-scores is an explicit assumption, not a derived result. LED V_f drift is typically slow-and-monotonic, while driver capacitor wear is often slow-then-knee; therefore, equal-magnitude early driver degradation could plausibly indicate higher near-term failure risk than equal LED drift. Keep symmetric weighting for now due to absent failure-time labels.
...
6. **Failure-time labels** (time-to-failure or replacement dates) — calibrate asymmetric weighting between LED and driver sub-scores instead of assuming symmetry.
